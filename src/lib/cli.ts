...
const separator = OPTIONS.abbreviate ? "," : ", ";

function getChamberChunks(chamber: PlayerState["chamber"]): slChunk[] {
  ...
  quarkCounts.forEach(({ count, flavor }) => {
    if (count > 0 || OPTIONS.showEmpty) {
      const flavorString = OPTIONS.abbreviate
        ? flavor.slice(0, 1)
        : ` ${flavor}`;
      chunks.push({
      ...
      chunks.push({ text: separator, style: "gray" });
    }
  });
...
function getPlayerChunks(player: PlayerState, highlight?: boolean): slChunk[] {
  const chunks: slChunk[] = [];
  ...
  // Chamber
  const chamberChunks = getChamberChunks(player.chamber);
  chunks.push(...chamberChunks);

  // Hadrons
  if (player.score > 0 || OPTIONS.showEmpty) {
    ...
    chunks.push({
      text: `${player.score}${OPTIONS.abbreviate ? "h" : " hadrons"}`,
      style: "bold",
    });
  }
...
function getStateChunks(state: CurrentGameState | PastGameState): slChunk[] {
  const chunks: slChunk[] = [];
  ...
  state.activeQuark.forEach((flavor, index, superposition) => {
    const flavorString = OPTIONS.abbreviate ? flavor.slice(0, 1) : flavor;
    chunks.push({
      text: flavorString,
      style: QUARK_MAPPING[flavor],
    });
    ...
    if (superposition.length > 1 && isNotLastElement) {
      if (OPTIONS.abbreviate) {
        ...
        if (index + 2 === superposition.length) {
          chunks.push({ text: `${separator}or `, style: "gray" });
        } else {
          chunks.push({ text: separator, style: "gray" });
        }
      }
  ...
  // Next line
  chunks.push({ text: OPTIONS.abbreviate ? "\n" : "\n\n" });

  // Players
  state.players.forEach((player) => {
    chunks.push(
      ...getPlayerChunks(player, player.order === state.activePlayer),
    );
    chunks.push({ text: "\n" });
  });
